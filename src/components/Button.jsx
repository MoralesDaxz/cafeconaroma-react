...
export const Button = ({ text, style, key, id, name, price, img, add, bg }) => {
  ...
  const productExisting = infoLocalStorage.find(
    (item) => item.nombre === buy.nombre
  );
  ...
  const putLocalStorage = () => {
    ...
    if (add === "addCoffee" || add === "suma") {
      ...
      if (productExisting.cantidad >= 1) {
        ...
        productExisting.cantidad < 1 ||
        productExisting.cantidad === 0
      ) {
        localStorage.setItem(
          "buys",
          JSON.stringify(infoLocalStorage.filter((item) => item.cantidad !== 0))
        );
      }
      localStorage.setItem(
        "buys",
  ...
  return (
    <div
      onClick={(evt) => {
        evt.target.textContent === "Agotado"
          ? alert("Pronto lo tendremos disponible")
          : putLocalStorage();
      }}
      id={id}
      key={id}
